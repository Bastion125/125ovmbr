...
async function loadPersonnel() {
    ...
    // Шукаємо personnelContent або personnelGrid
    let content = document.getElementById('personnelContent');
    if (!content) {
        content = document.getElementById('personnelGrid');
    }
    if (!content) {
        console.error('personnelContent or personnelGrid element not found in DOM');
        return;
    }
    ...
    try {
        ...
        // Перевіряємо, чи є personnelGrid (структура training.html)
        const personnelGrid = document.getElementById('personnelGrid');
        if (personnelGrid) {
            // Використовуємо структуру training.html - рендеримо тільки grid
            if (personnelList.length === 0) {
                personnelGrid.innerHTML = '<div class="empty-state">Особовий склад відсутній. Використайте форму вище для додавання.</div>';
            } else {
                personnelGrid.innerHTML = renderPersonnelGrid(personnelList);
            }
            // Зберігаємо дані для фільтрів
            window.allPersonnel = personnelList;
        } else {
            // Використовуємо стару структуру з personnelContent
            if (personnelList.length === 0) {
        ...
        const errorMsg = `<div class="error">Помилка завантаження особового складу: ${error.message || 'невідома помилка'}</div>`;
        if (content) {
            content.innerHTML = errorMsg;
        }
        const personnelGrid = document.getElementById('personnelGrid');
        if (personnelGrid) {
            personnelGrid.innerHTML = errorMsg;
        }
...
async function loadUserPersonnel() {
    try {
        ...
        if (data.user.personnel) {
            if (content) {
                if (content.id === 'personnelGrid') {
                    // Використовуємо структуру training.html
                    content.innerHTML = renderPersonnelGrid([data.user.personnel]);
                } else {
            ...
            if (content) {
                content.innerHTML = '<div class="empty-state">Персональна картка не знайдена</div>';
            }
        }
    } catch (error) {
...
function renderPersonnel(personnel) {
    ...
    if (!content) {
        console.error('personnelContent or personnelGrid element not found in renderPersonnel');
        return;
    }
