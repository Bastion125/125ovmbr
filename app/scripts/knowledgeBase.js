...
function renderKnowledgeBase(categories, materials) {
    ...
    // Застосовуємо останній пошуковий запит (якщо є)
    if (lastKnowledgeSearch) {
        filterKnowledgeMaterials(lastKnowledgeSearch);
    }
}
...
/**
 * Простий клієнтський пошук по вже відображеним матеріалам.
 * Фільтрує елементи в DOM за текстом заголовку/опису.
 */
function filterKnowledgeMaterials(query) {
    lastKnowledgeSearch = (query || '').toLowerCase();

    const list = document.getElementById('knowledgeMaterialsList');
    if (!list) return;

    const items = list.querySelectorAll('.knowledge-material-card, .knowledge-material-item');
    if (!items.length) return;

    items.forEach(item => {
        const text = item.textContent.toLowerCase();
        const match = !lastKnowledgeSearch || text.includes(lastKnowledgeSearch);
