...
async function loadCrews() {
    // Шукаємо crewsContent або crewsList
    let content = document.getElementById('crewsContent');
    if (!content) {
        content = document.getElementById('crewsList');
    }
    if (!content) {
        console.error('crewsContent or crewsList element not found in DOM');
        return;
    }

    try {
        // Завантажуємо типи БПЛА для форми (якщо форма є на сторінці)
        await loadEquipmentTypesForCrew();
        
        const response = await api.getCrews();
        const data = await api.handleResponse(response);
        crewsList = data.data || [];
        
        // Якщо є crewsList (структура training.html), рендеримо туди
        const crewsListEl = document.getElementById('crewsList');
        if (crewsListEl) {
            if (crewsList.length === 0) {
                crewsListEl.innerHTML = '<div class="empty-state">Екіпажі відсутні. Використайте форму вище для додавання.</div>';
            } else {
                crewsListEl.innerHTML = renderCrewsList(crewsList);
            }
        } else {
        ...
        const errorMsg = '<div class="empty-state">Помилка завантаження екіпажів</div>';
        if (content) {
            content.innerHTML = errorMsg;
        }
        const crewsListEl = document.getElementById('crewsList');
        if (crewsListEl) {
            crewsListEl.innerHTML = errorMsg;
        }
    }
}

// Відображення екіпажів (для старої структури)
...
// Відображення списку екіпажів (для структури training.html)
function renderCrewsList(crews) {
    if (crews.length === 0) {
        return '<div class="empty-state">Екіпажі відсутні</div>';
    }
...
async function loadEquipmentTypesForCrew() {
    try {
        ...
        if (!select) {
            console.warn('crewUavType select not found');
            return;
        }
        ...
        // Також завантажуємо типи засобів (equipment types)
        let equipmentTypes = [];
        try {
        ...
        // Збираємо типи БПЛА з засобів (type_uav)
        const uavTypesFromEquipment = [];
        if (data.data && Array.isArray(data.data)) {
            data.data.forEach(eq => {
                if (eq.type_uav && eq.type_uav.trim() !== '') {
                    uavTypesFromEquipment.push(eq.type_uav.trim());
                }
            });
        }
        
        // Додаємо назви типів засобів
        const typeNames = equipmentTypes.map(type => type.name).filter(name => name);
        
        // Об'єднуємо всі типи та видаляємо дублікати
        const allTypes = [...new Set([...uavTypesFromEquipment, ...typeNames])];
        
        console.log('Loading UAV types for crew form:', allTypes.length, 'types found');
        
        // Заповнюємо select
        select.innerHTML = '<option value="">Виберіть тип БПЛА</option>' +
            allTypes.map(type => `<option value="${(type || '').replace(/"/g, '&quot;')}">${(type || '').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</option>`).join('');
        
        // Якщо редагуємо екіпаж, встановлюємо значення
        const crewId = document.getElementById('crewId');
        if (crewId && crewId.value) {
            const crew = crewsList.find(c => c.id === parseInt(crewId.value));
            if (crew && crew.uav_type) {
                select.value = crew.uav_type;
            }
        }
    } catch (error) {
        console.error('Error loading equipment types for crew:', error);
        const select = document.getElementById('crewUavType');
        if (select) {
            select.innerHTML = '<option value="">Помилка завантаження типів</option>';
        }
    }
}
...
// Обробка форми екіпажу з training.html
async function handleCrewFormSubmit(event) {
    ...
    if (!name) {
        if (typeof showNotification === 'function') {
            showNotification('Введіть назву екіпажу', 'error');
        }
        return;
    }
    
    if (!uavTypeSelect || !uavTypeSelect.value) {
        if (typeof showNotification === 'function') {
            showNotification('Виберіть тип БПЛА', 'error');
        }
        return;
    }
    
    const uavType = uavTypeSelect.value;
    
    // Отримуємо вибрані члени екіпажу з форми
    const selectedMembers = [];
    const selectedCrewMembersEl = document.getElementById('selectedCrewMembers');
    if (selectedCrewMembersEl) {
        const memberCards = selectedCrewMembersEl.querySelectorAll('.crew-member-card');
        memberCards.forEach(card => {
            const memberId = card.getAttribute('data-personnel-id');
            const role = card.querySelector('.crew-member-role')?.value || '';
            if (memberId) {
                selectedMembers.push({
                    personnel_id: parseInt(memberId),
                    role: role
                });
            }
        });
    ...
    try {
        ...
        if (result.success) {
            if (typeof showNotification === 'function') {
                showNotification(crewId ? 'Екіпаж оновлено' : 'Екіпаж додано', 'success');
            }
            resetCrewForm();
            await loadCrews();
        }
...
async function handleAddCrew(event) {
    ...
    const uavTypeSelect = document.getElementById('crewUavType');
    const uavTypeCustom = document.getElementById('crewUavTypeCustom');
    const uavType = uavTypeSelect && uavTypeSelect.value === 'custom' ? (uavTypeCustom ? uavTypeCustom.value : '') : (uavTypeSelect ? uavTypeSelect.value : '');
...
function resetCrewForm() {
    ...
    const selectedCrewMembers = document.getElementById('selectedCrewMembers');
    if (selectedCrewMembers) {
        selectedCrewMembers.innerHTML = '';
    }
    const personnelDropdown = document.getElementById('personnelDropdown');
    if (personnelDropdown) {
        personnelDropdown.innerHTML = '';
        personnelDropdown.style.display = 'none';
    }
}
...
if (typeof window !== 'undefined') {
    ...
    window.handleCrewFormSubmit = handleCrewFormSubmit;
    ...
    window.resetCrewForm = resetCrewForm;
    window.loadEquipmentTypesForCrew = loadEquipmentTypesForCrew;
