...
function getRoleName(role) {
    // Використовуємо універсальну функцію getRoleName з utils/roles.js, якщо вона доступна
    // і не вказує на цю ж саму функцію (щоб уникнути рекурсії)
    if (typeof window !== 'undefined'
        && typeof window.getRoleName === 'function'
        && window.getRoleName !== getRoleName) {
        return window.getRoleName(role);
    }
    const roleNames = {
        'User': 'Користувач',
        'Readit': 'Інструктор',
